...
<?php
          ...
          $queryb = "SELECT MAX(Id) FROM solicitud";
		  $resulto = $con->query($queryb);
		  
		 		 if ($rowb = mysqli_fetch_row($resulto))
                     {
                        $idb = trim($rowb[0]);
                     }
         ...
         $query= "SELECT * FROM solicitud where ID = $idb"; 
         $result = mysqli_query ($con,$query);
		 $totalf = mysqli_num_rows($result);
		 mysqli_close($con);
...
'&nbsp;Y  FECHA DE NACIMIENTO:&nbsp;' .$Dateb.'</td>';        
echo '</tr>';
...
<p align="right"><button onclick="window.location.href='http://192.168.39.31/intranet/index.php/solicitud-actas-de-nacimiento'">Nuevo</button>
