...
//Empieza agregar remitente texto
	
	if  (isset($_POST['remitente']) and !empty($_POST['remitente']))
	{
	      $host = "localhost";
          $user = "root";
          $pw = "";
...
$rsql = mysqli_query($con,$sql); 	
		...
		$referencia == ""	
		...
		
		 {
		  ...
		  $queryb = "SELECT MAX(Id) + 1 FROM gestion";
...
mysqli_close($con); 
		...
		isset($_POST['sem']) and !empty($_POST['sem']) and
		isset($_POST['local']) and !empty($_POST['local']) and 
		isset($_POST['referencia']) and !empty($_POST['referencia'])
